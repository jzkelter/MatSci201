...
to setup
  ...
  set temp-width-table table:make
  foreach (range 0 5.1 0.1)  [t ->
    table:put temp-width-table (precision t 1) (list 0 0) ; for each temp, hold the average width and the number of measurements we've made
  ]

  setup-atoms
  init-velocity
  if auto-increase-temp? [
    set temp 0.1
  ]
  reset-timer
  reset-ticks
...
to setup-atoms
  ...
  ask min-one-of atoms [xcor] [
    set pinned? true
    set shape "circle 2"
  ]

  set last-atom max-one-of atoms [xcor]
  ask last-atom [
    set heading 0
    pen-down
    set color white
...
to init-velocity
  ...
  ask atoms [
    set vx v-avg * positive-or-negative
  ]
end
...
to go
  increment-temp
  if last-temp != temp or every-nth-tick? 1500 [scale-velocities]
  ...
  update-temp-width-table
  set last-temp temp
  tick
end

to update-temp-width-table
  let t precision temp 1
  let old-avg first (table:get temp-width-table t)
  let prev-n-samples last (table:get temp-width-table t)
  let new-avg old-avg * (prev-n-samples / (prev-n-samples + 1)) + current-width / (prev-n-samples + 1)
  table:put temp-width-table t (list new-avg (prev-n-samples + 1))
end

to-report current-width
  report [xcor] of last-atom
end

to increment-temp
  if auto-increase-temp? and ticks > 1000 and every-nth-tick? 2000 and temp < 3.5 [
    ifelse (precision temp 1) = 0.1 [
      set temp 0.5
    ] [
      set temp precision (temp + 0.5) 1
    ]
    scale-velocities
  ]
end

to-report every-nth-tick? [nth]
  report (precision ticks 2 mod nth) = 0
end
...
to-report current-temp
  report (1 / (3 * Kb)) * mean [vx ^ 2] of free-atoms
end
...
"" ""
PENS
"default" 1.0 2 -10649926 true "" "plotxy temp [xcor] of last-atom"
"pen-1" 1.0 0 -16777216 true "" "plot-pen-reset\nforeach table:to-list temp-width-table [tw ->\n if last last tw > 500 [\n    plotxy first tw (first last tw)\n   ]\n ]\n \n"
