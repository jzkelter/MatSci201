...
to-report calc-force [r]
  ...
  ; report calc-force-without-cutoff r - calc-force-without-cutoff cutoff-dist

end

to-report calc-force-without-cutoff [r]
  report eps * (n / (r ^ (n - 1)) - m / (r ^ (m - 1)))
end
...
to-report calc-r-min
  ; if sigma != 1 then use:  ((m / n) * sigma) ^ (1 / (m - n))
  report (m / n) ^ (1 / (m - n))
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot current-temp"
