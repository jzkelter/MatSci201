...
to setup
  ...
  ;create-monomers 200  [init-monomer]
  ;create-initiators num-initiators [init-initiator]
  create-monomers (monomer-to-initiator-ratio * 50) [init-monomer]
  create-initiators 50 [init-initiator]
...
to set-size
  set size sqrt ((1 + chain-length) / pi)
end

to go
  if ticks > add-reactants-until and  count radicals < 2 [stop]
  ...
  if ticks < add-reactants-until [
    create-monomers monomer-to-initiator-ratio [init-monomer]
    create-initiators 1 [init-initiator]
  ]
  tick
end
...
to arrange-by-size
  let max-size max [size] of turtles
  let min-size min [size] of turtles
  let med-size median [size] of turtles

  ask turtles [
    let jitter random-float 1
    if size = max-size [set jitter jitter - 2]
    set xcor min-pxcor + world-width * (size - min-size) / (max-size - min-size) + jitter
    set ycor random-pycor + random-float 1 - 0.5
  ]
end
...
PENS
"initiator" 1 0 -4079321 true "" "plot count initiators"
"monomer" 1 0 -10899396 true "" "plot count monomers"
"radical" 1 0 -2674135 true "" "plot count radicals"
"polymer" 1 0 -13345367 true "" "plot count polymers"
...
"" ""
PENS
"num-chains" 1 1 -13345367 true "" "every 0.5 [ do-histogram ]"
...
290
295
pause the model and then press this to arrange the molecules by size to see how they line up with the histogram
11
0
1
@#$#@#$#@
